...
public class Type_SQLite extends DBConnect {
    ...
    public Type_SQLite(Plugin plugin,String file_path) {
        this.plugin = plugin;
    ...
    public void createDBFile(){
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:"+FILE_PATH)) {
            ...
            plugin.getLogger().log(Level.SEVERE, "SQLiteデータベースに接続できませんでした。 エラーコード: " + var2.getErrorCode());
        }
    }
    ...
    public Connection open() {
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "SQLite用JDBCドライバーがこのマシンにインストールされていません");
        }
        return this.con;
    }
